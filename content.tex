...
\frame {
  ...
  \item[Definitional equality]\leavevmode\\
    ITT + \texteta.
  \item[Propositional equality]\leavevmode\\
    Proof relevance to 11. Equality proof is a path between values.
    No UIP but funext.
  \end{description}
}
...
\frame {
  ...
  ITT + impredicative \(Prop\) + inductive types.

  Propositional equality is Leibniz: \(\forall P : A \to Prop.\ P(a) \implies P(b) : Prop\).
  It is \textalpha\textbeta-equivalence. No \texteta since it's considered
  non-computational rule.
}
